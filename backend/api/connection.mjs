...
import { getSavedStatus } from './db.mjs';
...
app.get('/joblistings/:jobId/appliedStatus', async (req, res) => {
  const jobId = req.params.jobId;

  try {
    // Call the getAppliedStatus function to fetch the applied status
    const appliedStatus = await getAppliedStatus(jobId);

    // Send the applied status in the response
    res.status(200).json({ applied: appliedStatus });
  } catch (error) {
    // Handle errors
...
app.get('/joblistings/:jobId/savedStatus', async (req, res) => {
  const jobId = req.params.jobId;

  try {
    // Call the getAppliedStatus function to fetch the applied status
    const savedStatus = await getSavedStatus(jobId);

    // Send the applied status in the response
    res.status(200).json({ saved: savedStatus });
  } catch (error) {
    // Handle errors
    console.error("Error fetching applied status:", error);
    res.status(500).json({ error: 'Internal Server Error' });
